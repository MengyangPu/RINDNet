...
from modeling.rindnet import MyNet
...
class Trainer(object):
    ...
    def training(self, epoch):
        ...
        for i, (image, target) in enumerate(tbar):
            ...
            unet1, out_depth,out_normal,out_reflectance,out_illumination = self.model(image)
            focal_loss = self.criterion2(unet1, target[:, 0, :, :])
            loss = self.criterion([out_depth,out_normal,out_reflectance,out_illumination], target[:, 1:5, :, :])
            total_loss = 0.9 * focal_loss + 0.1 * loss
    ...
    def test(self, epoch):
        ...
        if not os.path.exists(self.output_dir):
            os.makedirs(self.output_dir)
        self.depth_output_dir = os.path.join(self.saver.experiment_dir, str(epoch+1), 'depth/met')
        if not os.path.exists(self.depth_output_dir):
            os.makedirs(self.depth_output_dir)
        self.normal_output_dir = os.path.join(self.saver.experiment_dir, str(epoch+1), 'normal/met')
        if not os.path.exists(self.normal_output_dir):
            os.makedirs(self.normal_output_dir)
        self.reflectance_output_dir = os.path.join(self.saver.experiment_dir, str(epoch+1), 'reflectance/met')
        if not os.path.exists(self.reflectance_output_dir):
            os.makedirs(self.reflectance_output_dir)
        self.illumination_output_dir = os.path.join(self.saver.experiment_dir, str(epoch+1), 'illumination/met')
