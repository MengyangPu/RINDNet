...
from modeling.decoder2 import build_decoder
...
class DeepLab(nn.Module):
    ...
    def forward(self, input):
        crop_h,crop_w = input.size(2),input.size(3)
        x, low_level_feat = self.backbone(input)
        x = self.aspp(x)  #([8, 256, 20, 20])
        [out_depth,out_normal,out_reflectance,out_illumination] = self.decoder(x, low_level_feat,crop_h,crop_w)   #([8, 16, 80, 80])
