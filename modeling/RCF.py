...
class RCF(nn.Module):
    def __init__(self):
        ...
        self.score_dsn1 = nn.Conv2d(21, 4, 1)
        self.score_dsn2 = nn.Conv2d(21, 4, 1)
        self.score_dsn3 = nn.Conv2d(21, 4, 1)
        self.score_dsn4 = nn.Conv2d(21, 4, 1)
        self.score_dsn5 = nn.Conv2d(21, 4, 1)
        # lr 0.001 0.002 decay 1 0
        self.score_final_depth = nn.Conv2d(5, 1, 1)
        self.score_final_normal = nn.Conv2d(5, 1, 1)
        self.score_final_reflectance = nn.Conv2d(5, 1, 1)
        self.score_final_illumination = nn.Conv2d(5, 1, 1)
    ...
    def forward(self, x):
        ...
        weight_deconv2 = make_bilinear_weights(4, 4).cuda()
        weight_deconv3 = make_bilinear_weights(8, 4).cuda()
        weight_deconv4 = make_bilinear_weights(16, 4).cuda()
        weight_deconv5 = make_bilinear_weights(32, 4).cuda()
        ...
        fusecat_depth = torch.cat(
            (so1[:, 0, :, :].unsqueeze(1), so2[:, 0, :, :].unsqueeze(1), so3[:, 0, :, :].unsqueeze(1)
             , so4[:, 0, :, :].unsqueeze(1), so5[:, 0, :, :].unsqueeze(1)), dim=1)
        fusecat_normal = torch.cat(
            (so1[:, 1, :, :].unsqueeze(1), so2[:, 1, :, :].unsqueeze(1), so3[:, 1, :, :].unsqueeze(1),
             so4[:, 1, :, :].unsqueeze(1), so5[:, 1, :, :].unsqueeze(1)), dim=1)
        fusecat_reflectance = torch.cat(
            (so1[:, 2, :, :].unsqueeze(1), so2[:, 2, :, :].unsqueeze(1), so3[:, 2, :, :].unsqueeze(1),
             so4[:, 2, :, :].unsqueeze(1), so5[:, 2, :, :].unsqueeze(1)), dim=1)
        fusecat_illumination = torch.cat(
            (so1[:, 3, :, :].unsqueeze(1), so2[:, 3, :, :].unsqueeze(1), so3[:, 3, :, :].unsqueeze(1),
             so4[:, 3, :, :].unsqueeze(1), so5[:, 3, :, :].unsqueeze(1)), dim=1)
        fuse_depth = self.score_final_depth(fusecat_depth)
        fuse_normal = self.score_final_normal(fusecat_normal)
        fuse_reflectance = self.score_final_reflectance(fusecat_reflectance)
        fuse_illumination = self.score_final_illumination(fusecat_illumination)
