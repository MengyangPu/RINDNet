...
from modeling.vgg16_c import VGG16_C
...
class BDCN(nn.Module):
    def __init__(self, pretrain=None, logger=None, rate=4):
        ...
        self.score_dsn1 = nn.Conv2d(21, 4, (1, 1), stride=1)
        self.score_dsn1_1 = nn.Conv2d(21, 4, 1, stride=1)
        ...
        self.score_dsn2 = nn.Conv2d(21, 4, (1, 1), stride=1)
        self.score_dsn2_1 = nn.Conv2d(21, 4, (1, 1), stride=1)
        ...
        self.score_dsn3 = nn.Conv2d(21, 4, (1, 1), stride=1)
        self.score_dsn3_1 = nn.Conv2d(21, 4, (1, 1), stride=1)
        ...
        self.score_dsn4 = nn.Conv2d(21, 4, (1, 1), stride=1)
        self.score_dsn4_1 = nn.Conv2d(21, 4, (1, 1), stride=1)
        ...
        self.score_dsn5 = nn.Conv2d(21, 4, (1, 1), stride=1)
        self.score_dsn5_1 = nn.Conv2d(21, 4, (1, 1), stride=1)
        self.upsample_2 = nn.ConvTranspose2d(4, 4, 4, stride=2, bias=False)
        self.upsample_4 = nn.ConvTranspose2d(4, 4, 8, stride=4, bias=False)
        self.upsample_8 = nn.ConvTranspose2d(4, 4, 16, stride=8, bias=False)
        self.upsample_8_5 = nn.ConvTranspose2d(4, 4, 16, stride=8, bias=False)
        #self.fuse = nn.Conv2d(40, 4, 1, stride=1)
        self.fuse_depth = nn.Conv2d(10, 1, 1, stride=1)
        self.fuse_normal = nn.Conv2d(10, 1, 1, stride=1)
        self.fuse_reflectance = nn.Conv2d(10, 1, 1, stride=1)
        self.fuse_illumination = nn.Conv2d(10, 1, 1, stride=1)
    ...
    def forward(self, x):
        ...
        fuse_depth = self.fuse_depth(
            torch.cat([p1_1[:, 0, :, :].unsqueeze(1), p2_1[:, 0, :, :].unsqueeze(1), p3_1[:, 0, :, :].unsqueeze(1),
                       p4_1[:, 0, :, :].unsqueeze(1), p5_1[:, 0, :, :].unsqueeze(1), p1_2[:, 0, :, :].unsqueeze(1),
                       p2_2[:, 0, :, :].unsqueeze(1), p3_2[:, 0, :, :].unsqueeze(1), p4_2[:, 0, :, :].unsqueeze(1),
                       p5_2[:, 0, :, :].unsqueeze(1)], 1))
        fuse_normal = self.fuse_normal(
            torch.cat([p1_1[:, 1, :, :].unsqueeze(1), p2_1[:, 1, :, :].unsqueeze(1), p3_1[:, 1, :, :].unsqueeze(1),
                       p4_1[:, 1, :, :].unsqueeze(1), p5_1[:, 1, :, :].unsqueeze(1), p1_2[:, 1, :, :].unsqueeze(1),
                       p2_2[:, 1, :, :].unsqueeze(1), p3_2[:, 1, :, :].unsqueeze(1), p4_2[:, 1, :, :].unsqueeze(1),
                       p5_2[:, 1, :, :].unsqueeze(1)], 1))
        fuse_reflectance = self.fuse_reflectance(
            torch.cat([p1_1[:, 2, :, :].unsqueeze(1), p2_1[:, 2, :, :].unsqueeze(1), p3_1[:, 2, :, :].unsqueeze(1),
                       p4_1[:, 2, :, :].unsqueeze(1), p5_1[:, 2, :, :].unsqueeze(1), p1_2[:, 2, :, :].unsqueeze(1),
                       p2_2[:, 2, :, :].unsqueeze(1), p3_2[:, 2, :, :].unsqueeze(1), p4_2[:, 2, :, :].unsqueeze(1),
                       p5_2[:, 2, :, :].unsqueeze(1)], 1))
        fuse_illumination = self.fuse_illumination(
            torch.cat([p1_1[:, 3, :, :].unsqueeze(1), p2_1[:, 3, :, :].unsqueeze(1), p3_1[:, 3, :, :].unsqueeze(1),
                       p4_1[:, 3, :, :].unsqueeze(1), p5_1[:, 3, :, :].unsqueeze(1), p1_2[:, 3, :, :].unsqueeze(1),
                       p2_2[:, 3, :, :].unsqueeze(1), p3_2[:, 3, :, :].unsqueeze(1), p4_2[:, 3, :, :].unsqueeze(1),
                       p5_2[:, 3, :, :].unsqueeze(1)], 1))

        fuse = torch.cat((fuse_depth, fuse_normal, fuse_reflectance, fuse_illumination), dim=1)
