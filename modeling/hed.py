...
class HED(nn.Module):
    ...
    def __init__(self, device):
        ...
        self.score_dsn1 = nn.Conv2d(64, 4, 1)  # Out channels: 1.
        self.score_dsn2 = nn.Conv2d(128, 4, 1)
        self.score_dsn3 = nn.Conv2d(256, 4, 1)
        self.score_dsn4 = nn.Conv2d(512, 4, 1)
        self.score_dsn5 = nn.Conv2d(512, 4, 1)
        self.score_final1 = nn.Conv2d(5, 1, 1)
        self.score_final2 = nn.Conv2d(5, 1, 1)
        self.score_final3 = nn.Conv2d(5, 1, 1)
        self.score_final4 = nn.Conv2d(5, 1, 1)
        #self.score_final = nn.Conv2d(20, 4, 1)

        # Fixed bilinear weights.
        self.weight_deconv2 = make_bilinear_weights(4, 4).to(device)
        self.weight_deconv3 = make_bilinear_weights(8, 4).to(device)
        self.weight_deconv4 = make_bilinear_weights(16, 4).to(device)
        self.weight_deconv5 = make_bilinear_weights(32, 4).to(device)
    ...
    def forward(self, x):
        ...
        fuse_cat_depth = torch.cat(
            (crop1[:, 0, :, :].unsqueeze(1), crop2[:, 0, :, :].unsqueeze(1), crop3[:, 0, :, :].unsqueeze(1),
             crop4[:, 0, :, :].unsqueeze(1), crop5[:, 0, :, :].unsqueeze(1)), dim=1)
        fuse_cat_normal = torch.cat(
            (crop1[:, 1, :, :].unsqueeze(1), crop2[:, 1, :, :].unsqueeze(1), crop3[:, 1, :, :].unsqueeze(1),
             crop4[:, 1, :, :].unsqueeze(1), crop5[:, 1, :, :].unsqueeze(1)), dim=1)
        fuse_cat_reflectance = torch.cat(
            (crop1[:, 2, :, :].unsqueeze(1), crop2[:, 2, :, :].unsqueeze(1), crop3[:, 2, :, :].unsqueeze(1),
             crop4[:, 2, :, :].unsqueeze(1), crop5[:, 2, :, :].unsqueeze(1)), dim=1)
        fuse_cat_illumination = torch.cat(
            (crop1[:, 3, :, :].unsqueeze(1), crop2[:, 3, :, :].unsqueeze(1), crop3[:, 3, :, :].unsqueeze(1),
             crop4[:, 3, :, :].unsqueeze(1), crop5[:, 3, :, :].unsqueeze(1)), dim=1)
        # print(fuse_cat.shape)
        fuse_depth = self.score_final1(fuse_cat_depth)  # Shape: [batch_size, 1, image_h, image_w].
        fuse_normal = self.score_final2(fuse_cat_normal)
        fuse_reflectance = self.score_final3(fuse_cat_reflectance)
        fuse_illumination = self.score_final4(fuse_cat_illumination)
