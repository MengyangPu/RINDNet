...
class DFF(BaseNet):
    ...
    def forward(self, x):
        ...
        side5 = self.side5(c5) # (N, 4, H, W)
        side5_w = self.side5_w(c5) # (N, 4*4, H, W)

        ada_weights = self.ada_learner(side5_w) # (N, 4, 4, H, W)
        ...
        for i in range(side5.size(1)-1):
            ...
            fuse = torch.cat((fuse, slice5, side1, side2, side3), 1) # (N, 4*4, H, W)

        fuse = fuse.view(fuse.size(0), self.nclass, -1, fuse.size(2), fuse.size(3)) # (N, 4, 4, H, W)
        fuse = torch.mul(fuse, ada_weights) # (N, 4, 4, H, W)
        fuse = torch.sum(fuse, 2) # (N, 4, H, W)
...
if __name__ == '__main__':
    model = DFF(4, backbone='resnet50')
    dummy_input = torch.rand(1, 3, 320, 480)
