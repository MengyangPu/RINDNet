...
class Trainer(object):
    ...
    def multiscale_test(self, epoch):
        ...
        if not os.path.exists(self.output_dir):
            os.makedirs(self.output_dir)
        self.depth_output_dir = os.path.join(self.saver.experiment_dir, str(epoch+1), 'depth/mat_ms')
        if not os.path.exists(self.depth_output_dir):
            os.makedirs(self.depth_output_dir)
        self.normal_output_dir = os.path.join(self.saver.experiment_dir, str(epoch+1), 'normal/mat_ms')
        if not os.path.exists(self.normal_output_dir):
            os.makedirs(self.normal_output_dir)
        self.reflectance_output_dir = os.path.join(self.saver.experiment_dir, str(epoch+1), 'reflectance/mat_ms')
        if not os.path.exists(self.reflectance_output_dir):
            os.makedirs(self.reflectance_output_dir)
        self.illumination_output_dir = os.path.join(self.saver.experiment_dir, str(epoch+1), 'illumination/mat_ms')
        if not os.path.exists(self.illumination_output_dir):
            os.makedirs(self.illumination_output_dir)
        self.model.eval()
        scale = [0.5, 1, 1.5]
        ...
        for i, image in enumerate(tbar):
            ...
            sio.savemat(os.path.join(self.illumination_output_dir, '{}.mat'.format(name)),
                        {'result': multi_fuse_illumination})
