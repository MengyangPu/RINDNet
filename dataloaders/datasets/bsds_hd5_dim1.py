...
def get_subwindow(im, label, center_pos, original_sz, avg_chans):
    ...
    if any([top_pad, bottom_pad, left_pad, right_pad]):
        te_label = np.zeros((r + top_pad + bottom_pad, c + left_pad + right_pad),
...
class Mydataset(data.Dataset):
    def __init__(self,root_path='/home/mpu/data/DOOBNet_data/Augmentation/', split='trainval',crop_size=513):
        self.split = split
...
if __name__ == '__main__':
    d = Mydataset(root_path='/home/mpu/data/DOOBNet_edge/Augmentation/', split='trainval',crop_size=320)
    print(len(d))
    for img,label in d:
        print(label.shape)
