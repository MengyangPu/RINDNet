...
def gt_flip(edge_ori):
    depth = edge_ori[0, 0, ...]
    normal = edge_ori[0, 1, ...]
    reflectance = edge_ori[0, 2, ...]
    illumination = edge_ori[0, 3, ...]
    ...
    edge_ori[0, 0, ...] = depth
    edge_ori[0, 1, ...] = normal
    edge_ori[0, 2, ...] = reflectance
    edge_ori[0, 3, ...] = illumination
...
def gt_rotate(edge_ori, times):
    ...
    edge_ori = np.zeros((1, 4, height, width), dtype=np.float32)
...
def BSDS__augmentation(mat_path, h5_dir, img_src_dir, img_dst_dir):
    ...
    illumination = gt[0][:, :, 3]
    height, width = depth.shape
    label = np.zeros((1, 4, height, width), dtype=np.float32)
    label[0, 0, ...] = depth
    label[0, 1, ...] = normal
    label[0, 2, ...] = reflectance
    label[0, 3, ...] = illumination
